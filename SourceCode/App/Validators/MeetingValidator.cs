...
namespace ModulesRegistry.Validators
{
    public class MeetingValidator : AbstractValidator<Meeting>
    {
        public MeetingValidator(IStringLocalizer<App> localizer)
        {
            ...
            RuleFor(m => m.PlaceName).NotEmpty().MaximumLength(50).IsOrdinaryText(localizer).WithName(n => localizer["Venue"]);
            RuleFor(m => m.Description).NotEmpty().MaximumLength(50).IsOrdinaryText(localizer).WithName(n => localizer[nameof(n.Description)]);
